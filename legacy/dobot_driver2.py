...
def connect(port: str = DEFAULT_PORT) -> bool:
    ...
    with _alat_lock:
        try:
            ...
            # bersihkan alarm dulu jaga-jaga kalau nyala merah
            clear_alarms()
            _require_device().speed(float(DEFAULT_SPEED_XY), float(DEFAULT_SPEED_Z))
            time.sleep(1.0)
            
            # masukkan ritual kalibrasi lokalisasi / homing di sini
            print(">>> Homing dlu ya bos, awas, clear area sekitar dobot, nggir ra minggir tabrak!")
            print(">>> Menunggu Homing-nya selesai (20 detik sih biasanya)...")
...
def move_to_absolute(
    ...
    with _alat_lock:
        ...
        try:
            controller._set_ptp_cmd(target_x, target_y, target_z, target_r, mode=PTPMode.MOVL_XYZ.value, wait=True)
        except Exception:
...
# ==========================================
# FUNGSI TAMBAHAN HMI: RESET & JOGGING
# ==========================================

# Titik Acuan "Siap" (Home Position)
HOME_X, HOME_Y, HOME_Z, HOME_R = 200.0, 0.0, 0.0, 0.0

def reset_home():
    """Mengembalikan Dobot ke posisi awal/siap."""
    global device
    
    if device is None:
        print("!!! Dobot belum konek, gabisa reset pose.")
        return False
    ...
    with _alat_lock:
        try:
            ...
            # masukkan ritual kalibrasi lokalisasi / homing di sini
            print(">>> Reset posisi dlu ya bos, awas, clear area sekitar dobot, nggir ra minggir tabrak!")
            print(">>> Menunggu Homing-nya selesai (20 detik sih biasanya)...")
            _require_device().home()
            time.sleep(20)

            print(">>> Dah beres resetnya, makasii udah nungguu")
...
def fast_jog(action, step=5.0):
    """Menggeser Dobot secara relatif berdasarkan arah panah HMI."""
    global device
    if not device: return

    with _alat_lock:
        try:
            # Ambil posisi mentah tanpa delay (agar tidak lag saat ditahan di HMI)
            p = device.get_pose()
            if p is None: return
            cx, cy, cz, cr = p.position.x, p.position.y, p.position.z, p.position.r
            ...
            if action == 'maju': cx += step
                ...
                # Bersihkan antrean perintah agar berhenti seketika
                device._set_queued_cmd_clear()
                return

            # Validasi dan eksekusi gerakan geser (Mode MOVL tanpa menunggu/wait=False agar mulus)
            if is_safe(cx, cy, cz):
                device._set_ptp_cmd(cx, cy, cz, cr, mode=PTPMode.MOVL_XYZ.value, wait=False)
            else:
                print("!!! [WARNING] Jog dibatalkan: Mentok batas limit mekanik.")
